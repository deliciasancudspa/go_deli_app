...
class _EarningsScreenState extends State<EarningsScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final orders = await _sb.from("orders").select("total, payment_method, status, created_at").eq("deliverer_id", rider.riderId).eq("status", "delivered").gte("created_at", from.toIso8601String());
      final payments = await _sb.from("rider_payments").select("*").eq("deliverer_id", rider.riderId).order("paid_at", ascending: false).limit(10);
      if (mounted) setState(() { _orders = List<Map<String, dynamic>>.from(orders); _payments = List<Map<String, dynamic>>.from(payments); _loading = false; });
  ...
  Widget build(BuildContext context) {
    final totalEarned = _orders.fold(0.0, (s, o) => s + ((o["total"] as num) * 0.15));
      ...
      body: _loading
        ...
        : RefreshIndicator(
            ...
            child: ListView(padding: const EdgeInsets.all(16), children: [
              Container(decoration: BoxDecoration(color: AppColors.surface, borderRadius: BorderRadius.circular(12), border: Border.all(color: AppColors.border)), child: Row(children: [
              ...
              Container(padding: const EdgeInsets.all(20), decoration: BoxDecoration(gradient: const LinearGradient(colors: [AppColors.primary, Color(0xFF2d1b69)], begin: Alignment.topLeft, end: Alignment.bottomRight), borderRadius: BorderRadius.circular(18)), child: Column(children: [
                ...
                Text("${_orders.length} pedidos completados", style: TextStyle(color: Colors.white.withOpacity(0.6), fontSize: 13)),
              ])),
              const SizedBox(height: 16),
              Row(children: [
                Expanded(child: _statCard("Efectivo recibido", _fmt(cashReceived), AppColors.warning, "Descontado de tu pago")),
                const SizedBox(width: 12),
                Expanded(child: _statCard("A depositar", _fmt(toDeposit), AppColors.success, "Transferencia pendiente")),
              ]),
              const SizedBox(height: 24),
              const Text("Pagos recibidos", style: TextStyle(fontSize: 16, fontWeight: FontWeight.w800)),
              const SizedBox(height: 12),
              if (_payments.isEmpty)
                Container(padding: const EdgeInsets.all(32), decoration: BoxDecoration(color: AppColors.surface, borderRadius: BorderRadius.circular(16), border: Border.all(color: AppColors.border)), child: const Column(children: [Text("💰", style: TextStyle(fontSize: 40)), SizedBox(height: 12), Text("Sin pagos registrados aun", style: TextStyle(color: AppColors.textLight, fontWeight: FontWeight.w600))]))
              else
  ...
  Widget _periodBtn(String label, String value) => Expanded(
    child: GestureDetector(
      ...
      child: Container(padding: const EdgeInsets.symmetric(vertical: 12), decoration: BoxDecoration(color: _period == value ? AppColors.accent : Colors.transparent, borderRadius: BorderRadius.circular(12)), child: Text(label, textAlign: TextAlign.center, style: TextStyle(fontWeight: FontWeight.w800, color: _period == value ? Colors.white : AppColors.textMedium, fontSize: 14))),
    ),
  );
